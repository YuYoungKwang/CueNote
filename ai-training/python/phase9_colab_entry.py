...
from cuenote_ai.colab_pipeline import prepare_dataset, prepare_environment, train_symbol_overfit, train_symbol_tile_overfit, train_symbol_tile_train, train_task


def main() -> None:
    parser = argparse.ArgumentParser(description="CueNote Phase 9E-H Colab pipeline")
    parser.add_argument("--run-mode", default=os.environ.get("CUENOTE_RUN_MODE", "SMOKE"), choices=["SMOKE", "LAYOUT_TRAIN", "SYMBOL_TRAIN", "SYMBOL_OVERFIT", "SYMBOL_TILE_OVERFIT", "SYMBOL_TILE_TRAIN", "FULL_PIPELINE"])
    ...
    elif args.run_mode == "SYMBOL_TILE_TRAIN":
        train_symbol_tile_train(REPO_ROOT, drive_root, args.run_mode)
